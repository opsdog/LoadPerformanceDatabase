/*
  prog to create the vxcalc_dm table once the raw data is loaded
...
main (int argc, char *argv[])
{
  ...
  */
  ...
  if ( TableCreate ) {
    strcpy(Command,"drop table if exists vxcalc_dm");
  ...
  strcpy(Command,"select distinct(serverid) from vxstat where vxtype = 'dm' order by serverid");
  CommandLength=strlen(Command);
  ...
  if ( (ServerQueryRow=mysql_fetch_row(ServerQueryResult)) != NULL ) {
	...
	strcat(Command," and vxtype = 'dm' order by esttime");
	CommandLength=strlen(Command);
      */

      sprintf(Command,"select distinct(esttime) from vxstat where serverid = %s and vxtype = 'dm' and esttime not in (select distinct esttime from vxcalc_dm where serverid = %s)",
	      ServerID, ServerID);
      CommandLength=strlen(Command);
	...
	while (EsttimeQueryRow != NULL) {
	  ...
	  sprintf(Command, "select sum(readops), sum(writops), sum(readblk), sum(writblk), avg(readms), avg(writms), max(readms), max(writms) from vxstat where vxtype = 'dm' and serverid = %s and esttime = %s",
		 ServerID, Esttime);
	  CommandLength=strlen(Command);
	  ...
	  sprintf(Command, "select count(esttime) from vxstat where vxtype = 'dm' and serverid = %s and esttime = %s and readops != 0 and writops != 0",
		 ServerID, Esttime);
	  CommandLength=strlen(Command);
	  ...
	  sprintf(Command,"insert into vxcalc_dm ( serverid, esttime, readops_sum, writops_sum, readblk_sum, writblk_sum, readms_avg, writms_avg, readms_max, adevices, writms_max, servtime ) values ( %s, %s, %d, %d, %d, %d, %f, %f, %f, %d, %f, %f)",
		  ServerID, Esttime, 
