...
int main(int argc, char *argv[]) {
  ...
  if (( OutputCSV = fopen("HTC-RIOPsLoad.csv","a") ) == (FILE *)NULL ) {
	...
	strcpy(DataLDEVlun, strTEMP);
      ...
      */
      ...
      /*  
          ...
          a value of -4 also seems to be magick code for "no data collected"
      ...
      */
      ...
      /*  if ( strcmp(Token,"-3") == 0 )  */
      if ( Token[0] == '-' )
	strcpy(DataReadIOPs,"0");
      else
	strcpy(DataReadIOPs,Token);

      fprintf(OutputCSV,"%s,%s,%ld,%s\n",
	      DataArraySN, DataLDEV, DataEpoch, DataReadIOPs);
