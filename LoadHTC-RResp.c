...
int main(int argc, char *argv[]) {
  ...
  if (( OutputCSV = fopen("HTC-RRespLoad.csv","a") ) == (FILE *)NULL ) {
  ...
  */
    ...
    */
	...
	strcpy(DataLDEVlun, strTEMP);
      ...
      */
      ...
      /*  if ( strcmp(Token,"-3") == 0 )  */
      if ( Token[0] == '-' )
	strcpy(DataReadResponse,"0");
      else
	strcpy(DataReadResponse,Token);

      fprintf(OutputCSV,"%s,%s,%ld,%s\n",
	      DataArraySN, DataLDEV, DataEpoch, DataReadResponse);
  ...
  printf("\n");
  printf("\n");
  printf("LoadHTC-RResp:  Ending stats:\n");
