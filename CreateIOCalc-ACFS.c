/*
  prog to create the iocalc_acfs table once the raw data is loaded
  ...
  */
...
#ifdef DEBUG
  printf("CreateIOCalc-ACFS:  entry\n");
  printf("CreateIOCalc-ACFS:  %d args\n", argc);
#endif
  ...
  if ( argc == 1 )
    ...
    if ( strcmp(argv[1],"-c") == 0 ) {
      ...
      printf("usage:  CreateIocalc_Acfs [ -c ]\n");
      exit(1);
    }

#ifdef DEBUG
  printf("CreateIOCalc-ACFS:  arg processes\n");
#endif
...
#ifdef DEBUG
  printf("CreateIOCalc-ACFS:  Database is running on %s\n",DBLocation);
#endif

  if ( strcmp(DBLocation,"localhost")==0 ) {
#ifdef DEBUG
    printf("CreateIOCalc-ACFS:  setting localhost variables...\n");
#endif
...
#ifdef DEBUG
      printf("CreateIOCalc-ACFS:  setting big-mac variables...\n");
#endif
...
#ifdef DEBUG
	printf("CreateIOCalc-ACFS:  setting evildb variables...\n");
#endif
...
#ifdef DEBUG
  printf("CreateIOCalc-ACFS:  Using database %s\n",DBName);
#endif
...
#ifdef DEBUG
  printf("CreateIOCalc-ACFS:  Database open\n");
#endif

  /* *********************************************************

                    drop the iocalc_acfs table ?

     ********************************************************* */

  if ( TableCreate ) {
    strcpy(Command,"drop table if exists iocalc_acfs");
    CommandLength=strlen(Command);

#ifdef DEBUG
    printf("CreateIOCalc-ACFS:  dropping iocalc_acfs table (%d)\n", CommandLength);
#endif
...
#ifdef DEBUG
  printf("CreateIOCalc-ACFS:  sending server list select (%d)\n", CommandLength);
#endif
  ...
  if ( (ServerQueryRow=mysql_fetch_row(ServerQueryResult)) != NULL ) {
...
#ifdef DEBUG
      printf("CreateIOCalc-ACFS:    TOP OF SERVER LOOP\n");
#endif
	...
	sprintf(Command,"select distinct(esttime) from iostat where serverid = %s order by esttime",
      ...
      */
      ...
      sprintf(Command,"select distinct esttime from iostat where serverid = %s and devtype = 7 and esttime not in (select distinct esttime from iocalc_acfs where serverid = %s)",
	      ServerID, ServerID);

      CommandLength=strlen(Command);

#ifdef DEBUG
      printf("CreateIOCalc-ACFS:    sending esttime list select (%d)\n", CommandLength);
#endif
...
#ifdef DEBUG
      printf("CreateIOCalc-ACFS:    Query stored (esttime) - processing...\n");
#endif

      if ( (EsttimeQueryRow=mysql_fetch_row(EsttimeQueryResult)) != NULL ) {
	while (EsttimeQueryRow != NULL) {
#ifdef DEBUG
	  printf("CreateIOCalc-ACFS:    TOP OF ESTTIME LOOP\n");
#endif
	  ...
	  */
	  ...
	  sprintf(Command,"select sum(rs), sum(ws), sum(krs), sum(kws), sum(wait), sum(actv), avg(wsvct), avg(asvct), avg(pctw), avg(pctb), avg(avgread), avg(avgwrit), sum(asvct), sum(wsvct) from iostat where esttime = %s and serverid = %s and devtype = 7",
		  Esttime, ServerID);

	  CommandLength=strlen(Command);

#ifdef DEBUG
	  /*  printf("CreateIOCalc-ACFS:      About to send %s (%d)\n", Command, CommandLength);*/
	  printf("CreateIOCalc-ACFS:      About to send data query (%d)\n", CommandLength);
#endif
...
#ifdef DEBUG
	  printf("CreateIOCalc-ACFS:      Query stored (data 1) - processing...\n");
#endif
...
#ifdef DEBUG
	  else printf("DataQuery had NULL result\n");
#endif

#ifdef DATADEBUG
	  printf("CreateIOCalc-ACFS:          Data Dump:\n");
	  printf("CreateIOCalc-ACFS:            vrs_sum: %f\n", vrs_sum);
	  printf("CreateIOCalc-ACFS:            vws_sum: %f\n", vws_sum);
	  printf("CreateIOCalc-ACFS:            vkrs_sum: %f\n", vkrs_sum);
	  printf("CreateIOCalc-ACFS:            vkws_sum: %f\n", vkws_sum);
	  printf("CreateIOCalc-ACFS:            vwait_sum: %f\n", vwait_sum);
	  printf("CreateIOCalc-ACFS:            vactv_sum: %f\n", vactv_sum);
	  printf("CreateIOCalc-ACFS:            vwsvct_avg: %f\n", vwsvct_avg);
	  printf("CreateIOCalc-ACFS:            vasvct_avg: %f\n", vasvct_avg);
	  printf("CreateIOCalc-ACFS:            vpctw_avg: %f\n", vpctw_avg);
	  printf("CreateIOCalc-ACFS:            vpctb_avg: %f\n", vpctb_avg);
	  printf("CreateIOCalc-ACFS:            vavgread_avg: %f\n", vavgread_avg);
	  printf("CreateIOCalc-ACFS:            vavgwrit_avg: %f\n", vavgwrit_avg);
	  printf("CreateIOCalc-ACFS:            vasvct_sum: %f\n", vasvct_sum);
	  printf("CreateIOCalc-ACFS:            vwsvct_sum: %f\n", vwsvct_sum);
#endif
	  ...
	  */
	  ...
	  sprintf(Command,"select count(device) from iostat where esttime = %s and serverid = %s and devtype = 7 and asvct != 0.0",
		  Esttime, ServerID);
...
#ifdef DEBUG
	  /*  printf("CreateIOCalc-ACFS:      About to send %s (%d)\n", Command, CommandLength);  */
	  printf("CreateIOCalc-ACFS:      About to get active device count (%d)\n", CommandLength);
#endif
...
#ifdef DEBUG
	  printf("CreateIOCalc-ACFS:      Query stored (data 2) - processing...\n");
#endif
...
#ifdef DATADEBUG
	  printf("CreateIOCalc-ACFS:        Data Dump:\n");
	  /*  printf("CreateIOCalc-ACFS:          STRvnumactivedevices: %s\n", STRvnumactivedevices);  */
	  printf("CreateIOCalc-ACFS:          vnumactivedevices   : %d\n", vnumactivedevices);
#endif
...
#ifdef DATADEBUG
	  printf("CreateIOCalc-ACFS:        Calculated Data Dump:\n");
	  printf("CreateIOCalc-ACFS:          vasvct_avg_a: %f\n", vasvct_avg_a);
	  printf("CretaeIocalc_Acfs:          vwsvct_avg_a: %f\n", vwsvct_avg_a);
	  printf("CretaeIocalc_Acfs:          vwkperop    : %f\n", vwkperop);
	  printf("CretaeIocalc_Acfs:          vrkperop    : %f\n", vrkperop);
#endif
	  ...
	  sprintf(Command,"insert into iocalc_acfs ( serverid, esttime, rs_sum,  ws_sum, krs_sum, kws_sum, wait_sum, actv_sum, wsvct_avg, asvct_avg, pctw_avg, pctb_avg, avgread_avg, avgwrit_avg, asvct_avg_a, wsvct_avg_a, adevices, wkperop, rkperop ) values ( %s, %s, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %d, %f, %f )",
...
#ifdef DEBUG
	  /*  printf("CreateIOCalc-ACFS:      About to insert: \"%s\" (%d)\n", Command, CommandLength);  */
	  printf("CreateIOCalc-ACFS:      About to insert (%d)\n", CommandLength);
#endif
