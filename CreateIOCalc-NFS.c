...
main (int argc, char *argv[])
{
  ...
  */
  ...
  if ( TableCreate ) {
    strcpy(Command,"drop table if exists iocalcnfs");
    CommandLength=strlen(Command);

#ifdef DEBUG
    printf("CreateIOCalc:  dropping iocalcnfs table (%d)\n", CommandLength);
#endif
...
#ifdef DEBUG
  printf("CreateIOCalcNFS:  sending server list select (%d)\n", CommandLength);
#endif
  ...
  if ( (ServerQueryRow=mysql_fetch_row(ServerQueryResult)) != NULL ) {
...
#ifdef DEBUG
      printf("CreateIOCalcNFS:    TOP OF SERVER LOOP\n");
#endif
	...
	sprintf(Command,"select distinct(esttime) from iostat where serverid = %s order by esttime",
      ...
      */
      ...
      sprintf(Command,"select distinct esttime from iostat where serverid = %s and devtype = 4 and esttime not in (select distinct esttime from iocalcnfs where serverid = %s)",
	      ServerID, ServerID);

      CommandLength=strlen(Command);

#ifdef DEBUG
      printf("CreateIOCalcNFS:    sending esttime list select (%d)\n", CommandLength);
#endif
...
#ifdef DEBUG
      printf("CreateIOCalcNFS:    Query stored (esttime) - processing...\n");
#endif

      if ( (EsttimeQueryRow=mysql_fetch_row(EsttimeQueryResult)) != NULL ) {
	while (EsttimeQueryRow != NULL) {
#ifdef DEBUG
	  printf("CreateIOCalcNFS:    TOP OF ESTTIME LOOP\n");
#endif
	  ...
	  */
	  ...
	  sprintf(Command,"select sum(rs), sum(ws), sum(krs), sum(kws), sum(wait), sum(actv), avg(wsvct), avg(asvct), avg(pctw), avg(pctb), avg(avgread), avg(avgwrit), sum(asvct), sum(wsvct) from iostat where esttime = %s and serverid = %s and devtype = 4",
		  Esttime, ServerID);
	  ...
	  printf("CreateIOCalcNFS:      About to send data query (%d)\n", CommandLength);
#endif
...
#ifdef DEBUG
	  printf("CreateIOCalcNFS:      Query stored (data 1) - processing...\n");
#endif
...
#ifdef DATADEBUG
	  printf("CreateIOCalcNFS:          Data Dump:\n");
	  printf("CreateIOCalcNFS:            vrs_sum: %f\n", vrs_sum);
	  printf("CreateIOCalcNFS:            vws_sum: %f\n", vws_sum);
	  printf("CreateIOCalcNFS:            vkrs_sum: %f\n", vkrs_sum);
	  printf("CreateIOCalcNFS:            vkws_sum: %f\n", vkws_sum);
	  printf("CreateIOCalcNFS:            vwait_sum: %f\n", vwait_sum);
	  printf("CreateIOCalcNFS:            vactv_sum: %f\n", vactv_sum);
	  printf("CreateIOCalcNFS:            vwsvct_avg: %f\n", vwsvct_avg);
	  printf("CreateIOCalcNFS:            vasvct_avg: %f\n", vasvct_avg);
	  printf("CreateIOCalcNFS:            vpctw_avg: %f\n", vpctw_avg);
	  printf("CreateIOCalcNFS:            vpctb_avg: %f\n", vpctb_avg);
	  printf("CreateIOCalcNFS:            vavgread_avg: %f\n", vavgread_avg);
	  printf("CreateIOCalcNFS:            vavgwrit_avg: %f\n", vavgwrit_avg);
	  printf("CreateIOCalcNFS:            vasvct_sum: %f\n", vasvct_sum);
	  printf("CreateIOCalcNFS:            vwsvct_sum: %f\n", vwsvct_sum);
#endif
	  ...
	  */
	  ...
	  sprintf(Command,"select count(device) from iostat where esttime = %s and serverid = %s and devtype = 4 and asvct != 0.0",
		  Esttime, ServerID);
...
#ifdef DEBUG
	  /*  printf("CreateIOCalc:      About to send %s (%d)\n", Command, CommandLength);  */
	  printf("CreateIOCalcNFS:      About to get active device count (%d)\n", CommandLength);
#endif
...
#ifdef DEBUG
	  printf("CreateIOCalcNFS:      Query stored (data 2) - processing...\n");
#endif
	  ...
	  printf("CreateIOCalcNFS:          vnumactivedevices   : %d\n", vnumactivedevices);
#endif
...
#ifdef DATADEBUG
	  printf("CreateIOCalcNFS:        Calculated Data Dump:\n");
	  printf("CreateIOCalcNFS:          vasvct_avg_a: %f\n", vasvct_avg_a);
	  printf("CretaeIOCalcNFS:          vwsvct_avg_a: %f\n", vwsvct_avg_a);
	  printf("CretaeIOCalcNFS:          vwkperop    : %f\n", vwkperop);
	  printf("CretaeIOCalcNFS:          vrkperop    : %f\n", vrkperop);
#endif
	  ...
	  sprintf(Command,"insert into iocalcnfs ( serverid, esttime, rs_sum,  ws_sum, krs_sum, kws_sum, wait_sum, actv_sum, wsvct_avg, asvct_avg, pctw_avg, pctb_avg, avgread_avg, avgwrit_avg, asvct_avg_a, wsvct_avg_a, adevices, wkperop, rkperop ) values ( %s, %s, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %d, %f, %f )",
	  ...
	  printf("CreateIOCalcNFS:      About to insert (%d)\n", CommandLength);
#endif
